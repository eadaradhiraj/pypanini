    ...
    def _conjugate_at_stem_parasmai(self, stem_base: str, lakara: str, purusha: str, vacana: str) -> List[str]:
        ...
        if lakara == "lw":
            ...
            if raw == "tip":
                ...
                res = stem[:-1] + "A" + prat
                if raw == "mip" and ((("r" in stem_base or "R" in stem_base) and stem_base[-1:] in ("k", "K", "g", "G", "N", "p", "P", "b", "B", "m", "y", "r", "l", "v", "s", "z", "S")) or stem_base.endswith("kz") or (stem_base.startswith("B") and stem_base.endswith("z"))) and res.endswith("ni"):
                    res = res[:-2] + "Ri"
                return [res]
        elif lakara == "viDiliN":
            stem = stem_base + "a"
